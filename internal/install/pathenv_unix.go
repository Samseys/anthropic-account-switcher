...
// ourPathExport returns the export line under our marker in content. Only a
// marker immediately followed by our export line counts; a stray marker is
// treated as absent and cleaned on the next add.
...
func addUserPath(dir string) (string, error) {
	...
	if cur, found := ourPathExport(existing); found {
		...
		// Block points at a different dir (install location moved): remove the
		// stale block so we converge to one entry at the current dir.
...
// removeUserPath keys on the marker line alone (not dir), so a block left by an
// older install at a different location is also removed. The dir argument is
// unused on Unix; kept for parity with the Windows signature.
func removeUserPath(dir string) error {
	...
	for i := 0; i < len(lines); i++ {
		...
		// Drop the export line after the marker (whatever dir it targets)
		// and the blank separator before it.
...
// isOurPathExport guards against a malformed block eating an unrelated line
// that happens to follow our marker.
