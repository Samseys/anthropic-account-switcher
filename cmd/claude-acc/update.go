...
// Self-update over the GitHub Releases API. The release pipeline publishes one
// static binary per OS/arch plus a SHA256SUMS manifest; `update` discovers the
// latest release, downloads the asset for this platform, verifies it against
// SHA256SUMS, and atomically replaces the running binary in place.
...
func assetName() string {
	name := fmt.Sprintf("%s_%s_%s", bin, runtime.GOOS, runtime.GOARCH)
...
func httpGet(url string, timeout time.Duration) ([]byte, error) {
	...
	req.Header.Set("User-Agent", bin+"/"+versionString())
...
func cmdUpdate(checkOnly, force bool) error {
	...
	if err := replaceRunningBinary(self, data, 0o755); err != nil {
...
func updateCheckPath() string {
	return filepath.Join(profileDir, ".update-check.json")
}

// maybeNotifyUpdate prints a one-line "new version available" hint to stderr.
// The hint fires on *every* command (the caller already excludes --json), but
// the network check that feeds it is throttled to once per updateCheckInterval:
// the warning is driven by the last-known "latest" recorded in the cache, not
// by whether a check happened on this run. It is best-effort and silent on any
...
func readUpdateCheck() (updateCheck, bool) {
	var c updateCheck
	raw, ok := readFileOpt(updateCheckPath())
	if !ok {
		return c, false
	}
...
func writeUpdateCheck(latest string) {
	...
	_ = writeFileAtomic(updateCheckPath(), data, 0o644)
}
